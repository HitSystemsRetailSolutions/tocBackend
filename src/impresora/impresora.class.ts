...
export class Impresora {
  ...
  async imprimirTicket(idTicket: number, albaran = false) {
    ...
    if (ticket?.cesta?.indexMesa != undefined) {
      const m = mesasG.find(
        (mesa) => mesa._id == Number(ticket.cesta.indexMesa)
      );
  ...
  private enviarMQTT(
    encodedData,
    options,
    mqtturl: string = "hit.hardware/printer"
  ) {
  ...
  private async _venta(info, recibo = null) {
    ...
    const arrayImprimir = [
      ...
      { tipo: "logo", payload: "" },
      {
        tipo: "text",
        payload: info?.nota
          ? `\x1B\x45\x01 Nota Taula: ${info.mesa} \x1B\x45\x00`
          : "",
      },
      ...
      arrayImprimir.push(
        {
          ...
          payload:
            info.mesa == null
              ? ""
              : `Taula: ${info.mesa} | PAX (Clients): ${info.comensales}`,
        },
    ...
    if (!qrEnabled && numFactura) {
      ...
      if (
        !(nif.length > 10) &&
        !(params.licencia.toString().length > 10) &&
        !(numFactura.length > 20)
      )
        if (params?.nif && isVerifactuTicket)
          arrayImprimir.push(
            ...
            {
              tipo: "text",
              payload: "* Verificado por el sistema VERI*FACTU *",
            }
          );
    }
  ...
  async detallesTicket(
    ...
    for (let i = 0; i < arrayCompra.length; i++) {
      ...
      if (thereIsDtoTienda) {
        let dto =
          arrayCompra[i].descuentoTienda && arrayCompra[i].descuentoTienda > 0
  ...
  async imprimirCajaAsync(caja: CajaSincro) {
    try {
      ...
      // Array para acumular deuda generada y pagada por cliente
      const resumenDeudaClientes = {};
      ...
      for (let i = 0; i < arrayMovimientos.length; i++) {
        ...
        switch (arrayMovimientos[i].tipo) {
          ...
          case "SALIDA":
            if (
              arrayMovimientos[i].concepto == "DEUDA" ||
              arrayMovimientos[i].concepto == "DEUDA ALBARAN"
            ) {
              // Obtener nombreCliente
              const nombreCliente =
                arrayMovimientos[i].nombreCliente || "(Desconegut)";
              if (!resumenDeudaClientes[nombreCliente]) {
                resumenDeudaClientes[nombreCliente] = { generat: 0, pagat: 0 };
              }
              resumenDeudaClientes[nombreCliente].generat +=
                arrayMovimientos[i].valor;
            } else {
          ...
          case "ENTRADA_DINERO":
            if (
              ...
              resumenDeudaClientes[nombreCliente].pagat +=
                arrayMovimientos[i].valor;
            } else {
      ...
      const textResumenDeudasCliente = Object.entries(resumenDeudaClientes)
        .map(([nombre, datos]) => {
          const d = datos as { generat: number; pagat: number };
          const espacios = " ".repeat(nombre.length); // mismo largo que el nombre
          return ` ${nombre}\n  generat: ${d.generat.toFixed(
            2
          )} | saldat: ${d.pagat.toFixed(2)}\n`;
        })
        .join("\n");
      textoMovimientos += `\nTotal targeta: ${(
      ...
      const device = new escpos.Network("localhost");
      const printer = new escpos.Printer(device);
      const simple = parametros?.params?.cierreSimple == "Si";
      const diasSemana = [
        "Diumenge",
        "Dilluns",
        "Dimarts",
      ...
      if (!simple)
        ...
        buffer = buffer.concat([
          ...
          // Mostrar resumen de deuda por cliente
          { tipo: "text", payload: "" },
          {
            tipo: "text",
            payload: "Resum deutes per client d'aquesta caixa:",
          },
          {
            tipo: "text",
            payload: textResumenDeudasCliente
              ? textResumenDeudasCliente
              : " Cap deute generat o saldat en aquesta caixa.",
          },
          { tipo: "text", payload: "" },
  ...
  calcularImporteDescuentos(lista: CestasInterface["lista"]) {
    ...
    lista.forEach((producto) => {
      ...
      if (descuento.greaterThan(0)) {
        ...
        if (descuento.equals(100)) {
          ...
          const precioSinDescuento = subtotal.div(
            new Decimal(1).minus(descuento.div(100))
          );

          totalSinDescuentos = totalSinDescuentos.plus(precioSinDescuento);
          descuentoTotal = descuentoTotal.plus(
            precioSinDescuento.minus(subtotal)
          );
        }
      } else {
        const subtotal = new Decimal(producto.subtotal);
    ...
    return `Total sense descompte: ${totalSinDescuentos.toFixed(
      2
  ...
  async imprimirEncargo(encargo: EncargosInterface) {
    ...
    try {
      ...
      const arrayImprimir = [
        ...
        {
          tipo: "text",
          payload: duplicarRebuts ? "ENCÀRREC COPIA 1" : "ENCÀRREC",
        },
        { tipo: "size", payload: [0, 0] },
        { tipo: "align", payload: "LT" },
        { tipo: "text", payload: cabecera },
      ...
      if (duplicarRebuts) {
        arrayImprimir.push(
          ...
          { tipo: "align", payload: "CT" },
          { tipo: "size", payload: [1, 1] },
          { tipo: "text", payload: "ENCÀRREC COPIA 2" },
          { tipo: "size", payload: [0, 0] },
          { tipo: "align", payload: "LT" },
          { tipo: "text", payload: cabecera },
          {
          ...
          { tipo: "align", payload: "CT" },
          { tipo: "size", payload: [1, 1] },
          { tipo: "text", payload: "ENCÀRREC COPIA 3" },
          { tipo: "size", payload: [0, 0] },
          { tipo: "align", payload: "LT" },
          { tipo: "text", payload: cabecera },
          {
    ...
    try {
      ...
      let impresion = [
        ...
        ...productos.flatMap((item) => {
          const bloques = [];
          let u = item.unidades - (item.printed || 0);
          if (u === 0) return bloques;
          bloques.push({ tipo: "size", payload: [1, 0] });
          bloques.push({ tipo: "style", payload: "b" });

          bloques.push({
            tipo: "text",
            payload: `x${u} ${item.nombre}`,
          });
