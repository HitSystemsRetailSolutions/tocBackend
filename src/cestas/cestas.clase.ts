  ...
  async imprimirNotaKitchen(
    ...
    try {
      ...
      if (ticketsWithPrinter.length > 0) {
        try {
          const res = impresoraInstance.imprimiTicketComandero(
            ticketsWithPrinter,
            nombre || "TAULA: " + cesta.indexMesa + 1,
            trabajador,
            cesta.comensales
          );
    ...
    for (let i = 0; i < cesta.lista.length; i++) {
      ...
      if (cesta.lista[i].promocion) {
        ...
        let precioArt = cesta.lista[i].isPrecioFrontend
          ? cesta.lista[i].subtotal / cesta.lista[i].unidades // Use existing price from frontend
          : cliente && (cliente.albaran || cliente?.vip)
            ? articulo.precioBase
            : articulo.precioConIva;
        const artPrecioIvaSinTarifa = articulo.precioConIva;
