...
export class Encargos {
	setEncargo = async (encargo) => {
        ...
        const encargo_santAna = {
			id: await this.generateId(this.getDate(encargo.opcionRecogida, encargo.fecha, encargo.hora, 'YYYYMMDDHHmmss', encargo.amPm), encargo.idTrabajador, parametros),
            cliente: encargo.idCliente,
            data: this.getDate(encargo.opcionRecogida, encargo.fecha, encargo.hora, 'YYYY-MM-DD HH:mm:ss.S', encargo.amPm),
            estat: Estat.NO_BUSCADO,
            tipus: 2,
            anticip: encargo.dejaCuenta,
            botiga: parametros.licencia,
            periode: encargo.opcionRecogida === OpcionRecogida.REPETICION ? Periodo.PERIODO : Periodo.NO_PERIODO,
            dias: encargo.opcionRecogida === OpcionRecogida.REPETICION ? this.formatPeriode(encargo.dias) : 0,
            bbdd: parametros.database,
            productos: encargo.productos,
            idTrabajador: encargo.idTrabajador
		};
		// Mandamos el encargo al SantaAna
		const { data }: any = await axios.post("encargos/setEncargo", encargo_santAna);
		...
		return schEncargos
			...
			.then((ok: boolean) => {
				if (!ok)
					return { error: true, msg: 'Error al crear el encargo' };
				return { error: false, msg: 'Encargo creado' };
			})
			.catch((err: string) => ({ error: true, msg: err }));
	};
	private async generateId(formatDate: string, idTrabajador: string, parametros: ParametrosInterface): Promise<string> {
        return `Id_Enc_${formatDate}_${parametros.licencia}_${parametros.codigoTienda}_${idTrabajador}`;
    }
    private getDate(tipo: OpcionRecogida, fecha: string | null, hora: string | null, format: string, amPm: string | null) {
        if(tipo === OpcionRecogida.HOY && format !== 'YYYYMMDDHHmmss') {
            fecha = moment(Date.now()).format('YYYY-MM-DD');
            hora = moment(Date.now()).set({hour: amPm === 'am' ? 12 : 17, minute: 0}).format('HH:mm');
